...
class SupervisedMAE_p(nn.Module):
    def __init__(self, cfg=None, img_size=384, patch_size=16, in_chans=3,
        ...
        self.norm = norm_layer(embed_dim)
        embed_dim = 768 if encodings=='resnext' else 768
        ...
        joint_num = 25  # 对应 pose 中的关节点数量
        joint_positions = np.arange(joint_num)  # [0, 1, ..., 24]
        self.decoder_spatial_pos_embed = nn.Parameter(torch.from_numpy(get_1d_sincos_pos_embed_from_grid(decoder_embed_dim, joint_positions).astype(np.float32)), requires_grad=False)
        # self.decoder_spatial_pos_embed = nn.Parameter(
        #     torch.from_numpy(get_2d_sincos_pos_embed(decoder_embed_dim, spatial_tokens).astype(np.float32)),
        #     requires_grad=False)
    ...
    @torch.no_grad()
    def sincos_pos_embed(self, max_len=1000, embed_dim=768, n=10000):  #embed_dim=512, max_len=120
    ...
    def forward(self, vid, yi=None, thw=None, boxes=None, shot_num=1):
        ...
        if not self.use_precomputed:
            ...
            latent = vid    #  [1,575,768]
        t,h,w = thw[0]   # t23, h25, w1
        x = self.decoder_embed(latent)  #[1,575,512]
        ...
        target_size = t * h * w  # 23 * 25 * 1 = 575
        current_size = self.decoder_spatial_pos_embed.shape[1]  # 207

        if target_size != current_size:
            # 使用插值调整大小
            pos_embed = torch.nn.functional.interpolate(
                self.decoder_spatial_pos_embed.unsqueeze(0).transpose(1, 2),
                size=target_size,
                mode='linear',
                align_corners=False
            ).transpose(1, 2)+ torch.repeat_interleave(decoder_pos_embed_temporal,
                      h*w,
                      dim=1)  # [1, 575, 768]
        else:
            pos_embed = self.decoder_spatial_pos_embed.repeat(1, t, 1)+ torch.repeat_interleave(decoder_pos_embed_temporal,
                      h*w,
                      dim=1)
        ...
        if shot_num > 0:
            ...
            y = yi   #[1,392,512]->[1,392,768]

        else:  ### use 0-shot token if shot_num>0
            
            y = self.shot_token.unsqueeze(0).repeat(x.shape[0],1, 1).to(x.device)  ## zero-shot token repeat #[1,1568,512]
        ...
        for i, decode_head in enumerate(self.decode_heads):  ### self-attention blocks with windowed self-attention
            x = decode_head(x, attn_mask)  #[1,45,25,1,512]
        pose_feat = x
        if self.downsample is not None:
            x = self.downsample(x)
        x = self.map(x)    ### linear mapping  [1,45,25,1,1]
        x = einops.rearrange(x, 'b d h w c -> b c d h w')  #[1,1,45,25,1]

        x = x.squeeze(1).reshape(-1, t, h, w)  #[1,45,25,1]
        x = self.pool(x)  ### spatial average pooling  # [1,45,1,1]
        x = x.squeeze(-1).squeeze(-1)  # [1,45]
