...
class UCFRep(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if self.split == 'train':
            csv_path = f"datasets/ucf-rep_new/train.csv"
        else:
            csv_path = f"datasets/ucf-rep_new/val.csv"
        self.df = pd.read_csv(csv_path)
        self.df['density_map_sum'] = 0
        # self.df['type'] = self.df['name'].apply(lambda x: x.split('/')[-2])
        self.df['type'] = self.df['name'].apply(lambda x: x.split('_')[1])
    ...
    def __getitem__(self, index):
        ...
        video_name = self.df.iloc[index]['name'].replace('.avi', '.npz')
        type = self.df.iloc[index]['name'].split('_')[1]
        ...
        if np.random.rand() < self.threshold and type != 'other' and self.split == 'train':
            ...
            exemplar_video_name = select_example_video.replace('.avi', '.npz')
        else:
        ...
        if not self.select_rand_segment:
            ...
            return vid_tokens, example_rep, gt_density, actual_counts, self.df.iloc[index]['name'][:-4], list(vid_tokens[0].shape[-3:]), shot_num
