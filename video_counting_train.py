...
def get_args_parser():
    ...
    # Logging parameters
    parser.add_argument('--log_dir', default='./logs/1118_videodir',
                ...
                with tqdm(total=len(dataloader),bar_format=bformat,ascii='░▒█') as pbar:
                    for i, item in enumerate(dataloader):
                        ...
                        with torch.cuda.amp.autocast(enabled=True):
                            ...
                            y, video_feat = model(data, example, thw, shot_num=shot_num)
                ...
                if args.use_tensorboard:
                    ...
                    if phase == 'val':
                        ...
                        if total_loss_all / float(count) < best_loss:
                            ...
                            best_filename = f'best_1_{epoch:03d}_obo{off_by_one / count:.3f}_mae{mae / count:.3f}_obz{off_by_zero / count:.3f}_rmse{np.sqrt(mse / count):.3f}.pyth'
                            torch.save({
                                'epoch': epoch,
                        ...
                        torch.save({
                            'model_state_dict': model.state_dict(),
                            'optimizer_state_dict': optimizer.state_dict(),
                        }, os.path.join(args.save_path, f'{epoch:03d}_obo{off_by_one / count:.3f}_mae{mae / count:.3f}_obz{off_by_zero / count:.3f}_rmse{np.sqrt(mse / count):.3f}.pyth'))
