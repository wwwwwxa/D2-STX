...
class Rep_count(torch.utils.data.Dataset):
    ...
    def load_tokens(self,path,is_exemplar,bounds=None, lim_constraint=np.inf, id=None, cycle_start_id=0, count=None, shot_num=1, get_overlapping_segments=False, segment_id=0):
        ...
        try:
            tokens = np.load(path) # Load in format C x t x h x w
            import os
            ...
            tokens = np.load(path)
            # print(f'Loaded successfully, shape: {tokens.shape}')  # 调试信息
    ...
    def __getitem__(self, index):
        video_name = self.df.iloc[index]['name'].replace('.mp4', '.npy')
        ...
        if np.random.rand() < self.threshold and action_type != 'other' and self.split == 'train':   #### do this with probability 0.4
            ...
            exemplar_video_name = select_example_video.replace('.mp4', '.npy')   ### select exemplar from the selected video
        else:
        ...
        if T <= self.num_frames:
            ...
            start = random.choice(np.arange(0, T-self.num_frames, 128))
            end = start + self.num_frames  ## for taking 8 segments

        sampled_segments = vid_tokens[(start//128) : (end//128)]
        tvm = sampled_segments.shape()[-3:]
        sampled_segments = einops.rearrange(sampled_segments, 'C t h w -> (t h w) C')
        gt = gt_density[(start//8): (end//8)]

        return sampled_segments, example_rep, gt, gt.sum(), self.df.iloc[index]['name'][:-4], tvm, shot_num
