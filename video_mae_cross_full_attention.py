...
class SupervisedMAE(nn.Module):
    ...
    def forward(self, vid, yi=None, thw=None, boxes=None, shot_num=1):
        ...
        for i, decode_head in enumerate(self.decode_heads):  ### self-attention blocks with windowed self-attention
            x = decode_head(x, attn_mask)      #[1,45,3,3,512]

        video_feat = x

        if self.downsample is not None:
            x = self.downsample(x)
        x = self.map(x)    ### linear mapping  [1,45,3,3,1]
        x = einops.rearrange(x, 'b d h w c -> b c d h w')  #[1,1,45,3,3]

        x = x.squeeze(1).reshape(-1, t, h, w)  #[1,45,3,3]
